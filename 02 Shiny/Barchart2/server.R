...
shinyServer(function(input, output) { 
  ...
  dfbc1 <- eventReactive(input$click2, {
    ...
    if(online2() == "SQL") {
      ...
      tdf = query(
        data.world(propsfile = "www/.data.world"),
        dataset="jlee/s-17-dv-project-6", type="sql",
        query="select `Sub-Category`, Region, sum(`Shipping Cost`) as sum_shippingcost 
        from globalshipments
  ...
  output$barchartPlot1 <- renderPlot({ggplot(dfbc1(), aes(x=`Sub-Category`, y=sum_shippingcost)) +
      ...
      geom_text(mapping=aes(x=`Sub-Category`, y=sum_shippingcost, label=round(sum_shippingcost)),colour="black", hjust=-.5) +
      geom_text(mapping=aes(x=`Sub-Category`, y=sum_shippingcost, label=round(sum_shippingcost - window_avg_shippingcost)),colour="blue", hjust=-2) +
      # Add reference line with a label.
      geom_hline(aes(yintercept = round(window_avg_shippingcost)), color="red") +
      geom_text(aes( -1, window_avg_shippingcost, label = window_avg_shippingcost, vjust = -.5, hjust = -.25), color="red")
  })
